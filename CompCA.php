<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($Cid) || empty($Cname) || empty($Cemail) || empty($Cpassword)) {
            echo "<p class='error-message'>Please fill in all fields";
        } else {
            $stmt = mysqli_prepare($conn, "INSERT INTO Company (Cid, Cname, Cemail, Cpassword) 
                                           VALUES (?, ?, ?, ?)");

            // Adjust the types based on your database schema
            mysqli_stmt_bind_param($stmt, "ssss", $Cid, $Cname, $Cemail, $Cpassword);
...
<body>
    <div class="card">
        ...
        <form id="loginForm" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
            <div class="field">
                <input autocomplete="off" id="logID" placeholder="ID" class="input-field" name="Cid" type="text" required>
            </div>

            <div class="field">
                <input autocomplete="off" id="logName" placeholder="Name" class="input-field" name="Cname" type="text" required>
            </div>
            <div class="field">
